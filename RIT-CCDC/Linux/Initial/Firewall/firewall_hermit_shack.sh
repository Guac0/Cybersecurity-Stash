...
iptables -t mangle -A OUTPUT -p tcp -m multiport --dports 22 -m state --state NEW,ESTABLISHED -d 192.168.15.0/24 -j ACCEPT
iptables -t mangle -A INPUT -p tcp -m multiport --sports 22 -m state --state ESTABLISHED -s 192.168.15.0/24 -j ACCEPT
