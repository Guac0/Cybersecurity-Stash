...
iptables -t mangle -A INPUT -p tcp -m multiport --dports 22 -m state --state NEW -s 10.15.1.0/24 -m recent --set
iptables -t mangle -A INPUT -p tcp -m multiport --dports 22 -m state --state NEW -s 10.15.1.0/24 -m recent --update --seconds 60 --hitcount 6 -j LOG --log-prefix "SSH Rate Limit Hit, Dropping Packet: " 
iptables -t mangle -A INPUT -p tcp -m multiport --dports 22 -m state --state NEW -s 10.15.1.0/24 -m recent --update --seconds 60 --hitcount 6 -j DROP
